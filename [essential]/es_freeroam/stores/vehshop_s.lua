  ...
  TriggerEvent('es:getPlayerFromId', source, function(user)
    ...
    MySQL.Async.fetchAll("SELECT * FROM user_vehicle WHERE identifier = @username",{['@username'] = player}, function (result)
      if (result) then
        ...
        if count == 5 then
          TriggerClientEvent("es_freeroam:notify", source, "CHAR_SIMEON", 1, "Simeon", false, "Full garage!\n")
        else
          if (tonumber(user.money) >= tonumber(price)) then
            user:removeMoney((price))
            TriggerClientEvent('FinishMoneyCheckForVeh', source, name, vehicle, price)
            TriggerClientEvent("es_freeroam:notify", source, "CHAR_SIMEON", 1, "Simeon", false, "Good road!\n")
          else
            TriggerClientEvent("es_freeroam:notify", source, "CHAR_SIMEON", 1, "Simeon", false, "Insufficient funds!\n")
          end
        end
      else
        if (tonumber(user.money) >= tonumber(price)) then
