...
function MenuGarage()
    ...
    MenuTitle = "Garage"
    ClearMenu()
    Menu.addButton("Store a vehicle","RentrerVehicule",nil)
    Menu.addButton("Take a vehicle","ListeVehicule",nil)
...
Citizen.CreateThread(function()
    ...
    while true do
        ...
        if GetDistanceBetweenCoords(vente_location[1],vente_location[2],vente_location[3],GetEntityCoords(LocalPed())) < 5 and IsPedInAnyVehicle(LocalPed(), true) == false then
            drawTxt('~g~E~s~ To sell the vehicle at 50% of the price d\'purchase',0,1,0.5,0.8,0.6,255,255,255,255)
            if IsControlJustPressed(1, 86) then
