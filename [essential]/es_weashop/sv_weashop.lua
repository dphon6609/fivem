		...
		if (tonumber(user.money) >= tonumber(price)) then
			...
			MySQL.Async.fetchAll("SELECT * FROM user_weapons WHERE identifier = @username",{['@username'] = player}, function (result)
				...
				if (tonumber(max_number_weapons) > tonumber(nb_weapon)) then
					...
					TriggerClientEvent("es_freeroam:notify", source, "CHAR_AMMUNATION", 1, "AMMUNATION", false, "Have fun with these toys!\n")
				else
					TriggerClientEvent('ToManyWeapons',source)
					TriggerClientEvent("es_freeroam:notify", source, "CHAR_AMMUNATION", 1, "AMMUNATION", false, "You have reached the limit of weapons ! (max: "..max_number_weapons..")\n")
				end
			end)
		else
			-- Inform the player that he needs more money
			TriggerClientEvent("es_freeroam:notify", source, "CHAR_AMMUNATION", 1, "AMMUNATION", false, "Come back when you have more money !\n")
		end
	end)
end)
