    ...
    while true do
      ...
      if(IsNearBank() or IsNearATM()) then
        if (atBank == false) then
          TriggerEvent('chatMessage', "", {0, 255, 0}, "^0Press E to access the bank");
        end
        atBank = true
        if IsControlJustPressed(1, 38)  then -- IF INPUT_PICKUP Is pressed
          if (IsInVehicle()) then
            TriggerEvent('chatMessage', "", {255, 0, 0}, "^1You can not access the bank from your vehicle!");
          else
            if bankOpen then
              closeGui()
              bankOpen = false
...
AddEventHandler('bank:deposit', function(amount)
  if(IsNearBank() == true or depositAtATM == true and IsNearATM() == true or depositAnywhere == true ) then
    if (IsInVehicle()) then
      TriggerEvent("es_freeroam:notif", "~r~ You can not use atm in a vehicle")
    else
      TriggerServerEvent("bank:deposit", tonumber(amount))
    end
  else
    TriggerEvent("es_freeroam:notif", "~r~ You can only deposit in a bank")
  end
end)
...
AddEventHandler('bank:withdraw', function(amount)
  if(IsNearATM() == true or IsNearBank() == true or withdrawAnywhere == true) then
    ...
    TriggerEvent("es_freeroam:notif", "~r~ It is not a bank or an ATM")
  end
end)
...
AddEventHandler('bank:givecash', function(toPlayer, amount)
  if(IsNearPlayer(toPlayer) == true or giveCashAnywhere == true) then
    ...
    if (playing ~= false) then
      ...
      TriggerEvent("es_freeroam:notif", "~r~ Player is not online")
    end
  else
    TriggerEvent("es_freeroam:notif", "~r~ The player must be close to you")
  end
end)
